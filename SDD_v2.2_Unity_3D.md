...
| 角色身高 | 一律 **1.60 公尺**（§7.3.1 硬規格） | **移除固定值**，改為「依角色設定身高建模、1 unit = 1 m、腳底貼齊 y=0」（§7.3.2） | 首發 9 隻角色的身高是角色設定的一部分；固定值換不到任何技術好處，而縮放插值需要的是尺度正確、不是身高一致 |
| Addressables key | 寫死 `spirit_longshan` | `spirit_{spirit_id}` | 切片單一角色時看不出差別，9 隻就看得出 |
...
**輪詢間隔採分層，不是單一固定值**〔v2.2 修正，原為一律 10 秒〕：

| 情境 | 間隔 | 理由 |
|---|---|---|
| **近距**：200m 內有召喚點，或已在任一行政區圍欄內 | **5 秒** | 玩家走路約 1.4 m/s；10 秒 = 14 公尺，50m 召喚按鈕會延遲亮起，而那個延遲感覺得到 |
| **遠距**：其餘地圖移動 | **15 秒** | 從士林走到萬華的路上不需要高頻定位 |
| EncounterScene | 連續讀取 | 供漸顯插值 |
| 切背景或離開 MapScene | **停止**輪詢 | **不申請背景定位權限** |

近距門檻（200m）刻意大於感應半徑（150m），讓玩家跨進 150m **之前**就已經在高頻模式，門檻觸發不會被輪詢間隔延遲。

> ⚠️ 分層而非一律 5 秒的理由：定位子系統的耗電與取樣頻率大致成正比，而 AC23.4 訂了「地板機一次完整迴圈 ≤ 8%」且**尚未實測**。在玩家不可能觸發任何門檻的路段用 5 秒，是拿確定的電量換不存在的反應速度。

**行政區圍欄判定沿用同一個 GPS fix**（§20.4.5），不另開定位來源——它是同一次取樣上的另一個判定，不是新的定位子系統。
...
| **尺度** | **Unity 1 unit = 1 公尺**，依角色設定的真實身高建模〔**v2.2 修正**：不再要求所有角色同高，見 §7.3.2〕 |
...
| **尺度** | **Unity 1 unit = 1 公尺**，依該角色設定的真實身高建模〔**v2.2 修正**：原為「一律 1.60 公尺」，見 §7.3.2〕 |
| **面數** | **15,000–20,000 tris**（不得偷懶做低面數，否則 P0-D／AC16.2 效能驗證失真） |
| Addressables key | `spirit_{spirit_id}`（與正式資產同一 key，僅換 bundle 內容）。例：`spirit_longshan_temple` |
...
□ 面數 15,000–20,000 tris
□ 匯入後 Scale Factor = 1，模型高度等於該角色設定身高（±5 cm）
□ 腳底貼齊 y=0，面向 +Z
□ 材質 slot 為 Body / Hair / Eyes
□ prefab 節點路徑符合上方結構
□ 已打包為 Addressables，key = spirit_{spirit_id}
```

> **附帶價值**：Mixamo 免費角色不含任何 blend shape，因此製作灰模必須實際走一次「Blender 加 shape key → 分離眼睛材質 → 匯出 .fbx」，即正式產線第 ④ 步的預演。灰模若製作不順，正式角色只會更困難。

### 7.3.2 角色尺度規格〔v2.2 新增，取代固定身高〕

#### 為什麼原本要求 1.60m，以及為什麼現在不需要

v2.1 §7.3.1 訂「身高一律 1.60 公尺」，理由寫的是「距離縮放插值基準」。當時垂直切片只有一隻角色，固定值與尺度正確性剛好是同一件事，所以沒有分開。

**首發角色展開到 9 隻之後，這兩件事分開了**，而固定身高的那一半變成負擔：每隻靈魂的身高是**角色設定的一部分**（一位年邁、記憶衰退的女性形象，跟一位三百年的守望者，不該一樣高），硬拉成同高會讓角色設計失去一個表達維度，而且換不到任何技術好處。

#### 真正需要保證的是尺度正確，不是身高一致

| 保留（硬規格） | 移除 |
|---|---|
| **Unity 1 unit = 1 公尺** | ~~所有角色身高相同~~ |
| **腳底中心貼齊 y = 0，面向 +Z** | ~~固定 1.60m~~ |
| 匯入時 **Scale Factor = 1**，不靠 Unity 端縮放湊高度 | |
| 模型高度 = 該角色設定身高（±5 cm） | |

只要每隻角色都以真實比例建模、腳底對齊原點，§5.1 第 6 點的「角色縮放依 GPS 距離插值」就成立——那個插值縮放的是**觀看距離造成的視角大小**，不是把角色正規化成同一個高度。兩隻不同高的角色站在同樣距離看起來不一樣高，是正確的，不是 bug。

#### 身高的存放位置

每隻角色的設定身高寫在**該角色的人格卡／角色設定文件**（`citysoul-doc/character/`），與外型、服裝同層級，由臉與敘事共同決定。

**不進資料庫。** `spirits` 表的 `height_offset_m`（§10.2）是**擺放用的垂直偏移**（例如角色要站在台階上），與角色本身多高無關，兩者不可混用。

**建議範圍 1.5–1.9 公尺。** 這不是硬規格，是提醒：超出這個範圍時，2.5 公尺的觀看距離（`SpiritPlacement.distanceM`）與 1.55 公尺的視線高度會需要一併複審，否則會出現要抬頭仰望或低頭俯視才看得到臉的情況。

#### 佔位膠囊

`SpiritPlacement.characterHeightM` 目前預設 1.6，**僅用於生成佔位膠囊**（正式 prefab 是直接 `Instantiate`，不做任何高度正規化）。灰模到位後此欄位即無作用，保留預設值即可，不需要跟著每隻角色改。
...
### 10.6 v2.2 異動三：行政區圍欄判定

供 arc 入口使用（§20.4.5.1）。新增一支端點：

| Method | Path | 驗證 | 說明 |
|---|---|---|---|
| POST | `/api/v1/districts/check-entry` | Session Token | 收座標，回是否在某個行政區圍欄內；首次成立時一併發放該區 arc 的入口道具 |

**Request:**

```jsonc
{ "latitude": 25.0375, "longitude": 121.4998 }
```
...
```jsonc
{
  "district_id": "wanhua",          // 不在任何圍欄內時為 null
  "entry_granted": true,            // 本次呼叫是否首次成立並發放入口道具
  "arc_id": "wanhua_homeward"       // 無對應 arc 時為 null
}
```

⚠️ **座標算完即丟**，不寫入任何持久化欄位（`brain.districts` 的 docstring 已明訂圍欄判斷不留紀錄）。這與 `/sense`、`/summon` 同一原則，也受同一條 Schema 硬約束拘束：不得新增 `location_history` / `movement_trace` 類型的表。

**去重**：`entry_granted` 只在首次為 `true`，靠 `player_inventory` 的 `uq_inventory_player_item` 唯一索引，**不靠應用層先查再寫**（併發下會漏）。重複呼叫是正常的——輪詢每 5–15 秒一次、每次進 MapScene 也會呼叫一次。

**不在任何圍欄內不是錯誤**：回 `200` 與 `district_id: null`。大多數玩家在大多數時間都不在任何 arc 區域內。
...
| AC | 異動 |
|---|---|
| **AC11** 距離偵測省電方案 | 平台改為 Unity `Input.location`；輪詢間隔由 30 秒改為 10 秒（v2.1）→ **再改為分層 5 秒／15 秒（v2.2，見 §5.5）**；「不申請背景定位權限」不變 |
...
| **AC15.7** 〔v2.2 新增〕 | 資產匯入後 Scale Factor = 1，模型高度等於該角色設定身高（±5 cm），腳底貼齊 y=0、面向 +Z。**不驗證各角色之間身高是否一致**（§7.3.2） |
...
| POST | `/api/v1/districts/check-entry` | Session | 行政區圍欄判定 ＋ arc 入口道具發放（§10.6，v2.2 新增） |
...
- **AC11.1** 〔**v2.2 改寫**〕Given App 在地圖畫面且 200m 內有召喚點（或已在行政區圍欄內） When 檢查 GPS 存取模式 Then 為前景區間輪詢、間隔約 **5 秒**，**不要求背景定位權限**
- **AC11.1b** 〔**v2.2 新增**〕Given App 在地圖畫面且 200m 內無召喚點、亦不在任何圍欄內 When 檢查輪詢頻率 Then 間隔約 **15 秒**
- **AC11.1c** 〔**v2.2 新增**〕Given 玩家自遠距移動至近距 When 跨過 200m Then 輪詢間隔**自動升頻**，不需離開再重進地圖畫面
...
萬華主線〈回家的畫〉的完整內容與工程規格**不在本文件**，在 `citysoul-doc/story/`：

| 文件 | 是什麼 |
|---|---|
| `story/wanhua_district_storyline_aming_landmark_photo_v1.md` | **完整互動腳本**——劇情、任務、逐句對話、資料形狀草案、內容治理規則。內容作者與審核的正本 |
| `story/wanhua_story_runtime_architecture_v1.md` | **工程交付規格**——資料模型、API 增量、寫入順序、Unity 元件責任、QA 驗收 |
| `story/story_arc_writing_sop.md` | 寫新 arc 前的 SOP 與前置檢查表 |

本節（§20.4）只保留**跨 arc 通用的規則**；萬華專屬的內容與點位以那三份為準。兩者衝突時，通用規則（本節）優先，並回頭修正那三份。

> ⚠️ 檔名中的 `landmark_photo` 指「含**可選**景觀印記」，不是「必拍」。景觀拍攝在任何一站都不是完成條件（§20.4.5）。

#### 20.4.1 現況

`story_arcs`、`story_beats`、`resonance_unlockables`、`players_story_progress` 四張表已建立、ORM 已定義，`public.quests` 也已有 `quest_type IN ('daily','story','resonance_gated')` 與 `story_beat_id`。但**沒有任何讀寫邏輯**：沒有 beat 推進、沒有 `prerequisite_beat_ids` 因果鏈判定、沒有對外端點、prompt 組裝不知道玩家踩在哪個 beat 上。

內容面同樣是零：腳本已寫到逐句對話的程度，但**所有地標點位仍是暫定值**（待實勘），且沒有任何一筆進資料庫。
...
#### 20.4.4 MVP 範圍界線〔v2.2 定案〕

| 納入 MVP | 不納入 MVP |
|---|---|
| 三站主線 Beat、任務、結局 | 每日重訪任務與輪播 |
| 一頁個人化年代簿短札（≤ 90 字，三次選擇組合預寫片段） | 「萬華餘白」短札與其排程 |
| 親近度沿用每個靈魂既有的共鳴值 | 另建可扣分的好感系統 |
| 年代簿以 **UI 面板**呈現（§5.2 場景清單不變） | 年代簿獨立場景 |
| — | 觀光推廣／店家合作模組（見下方隔離規則） |

不納入的項目**不建立排程、任務目錄、API 或 UI**——留在文件裡當未來規劃即可，不佔用切片後的第一輪工。

**未來合作模組的隔離規則**（先寫進規格，避免日後被塞進主線）：只在玩家主動開啟的獨立頁籤顯示；不注入人格 prompt、角色台詞或歷史資訊卡；合作單位只能驗證一次性兌換碼，不得取得 `player_id`、故事選項、親近度、對話或位置資料；兌換不得成為任何故事或路線前置。

#### 20.4.5 arc 入口與拍照的通用規則〔v2.2 定案〕

| 規則 | 內容 |
|---|---|
| **arc 入口** | 由**首次進入該 arc `district_id` 的行政區圍欄**觸發〔**v2.2 定案**，見下方〕。重複觸發由 `player_inventory` 的唯一索引擋下 |
| **同區限制** | `/state` 的 `next_target` 只能是與 arc 同 `district_id` 的地標 |
| **靈魂可見性** | `unlock_spirit_id` 解鎖的是**主線標記與主線對話**，不是靈魂本身。所有靈魂一律在地圖上可見、可感應、可對話、可收藏 |
| **景觀拍攝** | **任何一站都不是完成條件。** 不拍、上傳失敗（無訊號／逾時／429）、`landmark_recognized=false`，三者都可正常交件，差別只在有無景觀印記，且都不得呈現為錯誤 |
| **無期限** | 可跨日、可中斷；未完成不降親近度、不失去道具、不重置已完成 Beat |

#### 20.4.5.1 arc 入口用行政區圍欄的落地細節

**入口道具在玩家踏進該行政區時就發**，不必先走到入口地標。理由是入口文件（信）的敘事定位是「留給這條街的信」——它屬於整個區，不屬於某一個地標。

**前置在本版已幾乎齊備**，不是從零開始：

| 前置 | 狀態 |
|---|---|
| 萬華區界 GeoJSON | ✅ `citysoul-backend/data/districts/wanhua.geojson` |
| PostGIS | ✅ migration `0006_postgis_extension` |
| 多邊形內含判定 | ✅ `app/modules/body/districts.py` 的 `is_point_in_polygon()` |
| 邊界載入與 `ST_IsValid` 驗證 | ✅ `scripts/load_districts.py` |
| `check_player_in_district()` 與對外端點 | ❌ 待做 |

**耗電成本接近零**：圍欄判定掛在 §5.5 既有的前景輪詢上，是**同一個 GPS fix 上的另一個判定**，不是新的定位來源。

**判定在伺服器端**，不下發多邊形給客戶端：36 KB 的邊界資料要下載且客戶端判定可偽造；伺服器端與 `/sense`、`/summon` 同一套慣例。座標算完即丟，不留任何紀錄。

**觸發時機**：除了輪詢，**進入 MapScene 時必須判定一次**。否則住在區內、但很少在移動中開 App 的玩家可能一直拿不到入口。

**邊界不算在內〔定案〕**：維持 `ST_Contains`，剛好落在區界線上的點判定為**不在區內**（OGC 定義，boundary 不屬於 interior，見 `districts.py` 的 docstring）。**不換 `ST_Covers`、不加 buffer。** 玩家不會停在數學線上，而下一次輪詢（5–15 秒）就會把真的走進來的人判進去。

**使用完整官方行政區界〔定案〕**：萬華區約 8.85 km²，**西門町也在區內**，捷運板南線穿越該區。因此「去西門逛街」或「開著 App 搭捷運經過萬華」都會發信——**這是要的行為**，不是副作用：信的敘事定位是「留給這條街的信」，屬於整個區。不縮小多邊形。

> **為什麼拍照不能是硬條件**：戶外收訊、配額與逾時會讓玩家「人在現場、照片也拍了，卻交不了件」。那違反 §18.7.3——這些是包裝，玩家的進度是真的。

#### 20.4.6 世界觀邊界（已在劇本內處理，此處記錄為規則）
