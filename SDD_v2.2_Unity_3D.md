...
| 規則 | 內容 |
|---|---|
| **arc 入口** | 由該 arc `entry_spirit_id` 的**首次 150m 感應驗證**（`POST /sense`）觸發，**不使用行政區多邊形圍欄**。重複觸發由 `player_inventory` 的唯一索引擋下 |
| **同區限制** | `/state` 的 `next_target` 只能是與 arc 同 `district_id` 的地標。這是**伺服器端的資料範圍檢查**，不需要玩家端圍欄 |
...
> **為什麼入口不用行政區圍欄**：圍欄換到的只是「早幾百公尺拿到入口道具」，代價是 `districts.boundary` 的 GeoJSON、`check_player_in_district()`（目前刻意未實作）、客戶端第三種前景判定，以及一支新的區界驗證端點。`/sense` 這條路上的到場驗證、token 與去重全部現成。
>
