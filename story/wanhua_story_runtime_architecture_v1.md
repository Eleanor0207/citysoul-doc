...
- 每個行政區有一個固定的 arc 起始地標；萬華為 `longshan_temple`。**入口由該地標的當次 150m 感應驗證觸發（`POST /sense`），不使用行政區多邊形圍欄。**
...
2. Arc 入口只能由 `arc.entry_spirit_id` 的**首次成功 `/sense` 感應驗證**建立；原始座標驗證後即丟棄，只持久化「已發放入口文件」的結果。重複觸發由 `player_inventory` 的唯一索引擋下。
3. `/state` 的 `next_target` 僅可回傳與 arc 同 `district_id` 的地標，並含可選 `direction_hint`；沒有任何「跨區下一站」欄位。`district_id` 是**伺服器端的資料範圍檢查**，不需要玩家端的區界圍欄。
...
- 萬華入口只在**龍山寺首次 150m 感應驗證**成立後啟動；其他地標（含同區的紅樓、剝皮寮）的感應或召喚不得發放萬華文件或推進萬華 Beat。玩家先到紅樓時可正常對話與收藏，但不會拿到信——這是預期行為。
...
- 龍山寺首次感應只發一次信；在 150m 邊緣來回走動不得重複發放（唯一索引驗證）。其他地標不得發放。中斷後重啟保留已確認的點位與物件。
