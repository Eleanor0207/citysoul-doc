...
| `StoryLetterView` | 全螢幕信件／畫作／**三個可看位置**（熱區＋可讀標籤）；**開啟即清除該項目的未讀標記**（不是看完才清）；三處皆可重看；看過一處即可往下走。 | 用對話氣泡或問句選單呈現（此節點無靈魂在場，也沒有人在應答）、看過後移除其他位置、強制看完三處、自動全螢幕彈出、只做熱區不給標籤。 |
...
3. **序章不是選單，是「看向哪裡」。** 人不會跟一封信對話；寫成問句而信給出回覆，等於信在應答。玩家點的是畫面位置，敘述者是年代簿。節點型別為 `look_points`（`exclusive: false`、`min_viewed_to_proceed: 1`），`story_focus` 以 `set_once` 由**第一個看的位置**寫入、之後不覆蓋。客戶端不得在看過後移除其他位置，也不得要求看完三處才放行。
